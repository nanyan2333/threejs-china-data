...
import { GeoJsonType, chartData } from "./typed"
...
function Map3D(props: Props) {
	const { geoJson, dblClickFn, projectionFnParam, chartData } = props
	...
	useEffect(() => {
		...
		labelRenderer.domElement.style.pointerEvents = "none" // ★★必须
		...
		// if (
		// 	chartRef?.current &&
		// 	!labelRenderer.domElement.contains(chartRef.current)
		// ) {
		// 	labelRenderer.domElement.appendChild(chartRef.current)
		// }
	...
	useEffect(() => {
		...
		if (chartData.length) {
			chartData.forEach((item: chartData, index: number) => {
				const echartDom = document.getElementById(`chart-${index}`)
				const myChart = echarts.init(echartDom)

				myChart.setOption({
					title: { text: item.name },
					tooltip: {},
					xAxis: { type: "category", data: item.time },
					yAxis: {},
					series: [{ type: "bar", data: item.value }],
				})
			})
		}
	}, [chartData])
